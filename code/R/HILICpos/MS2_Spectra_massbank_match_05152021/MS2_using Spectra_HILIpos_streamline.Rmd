...
note: "Don't do retention range as they are run in different system"
...
```{r}
output_dir1 = "../../../../data/output/MS2_Spectra_massbank_match_05152021/HILICpos/massbank_search_agst_Sign_list/"
...
for (i in (1:length(sps_select_l))) { 
  ...
  if(max(res_l[[i]]) > threshold) {mbank_best_match <- mbank_with_mz[idx[2]]}
...
output_dir = "../../../../data/output/MS2_Spectra_massbank_match_05152021/HILICpos/massbank_search_agst_Sign_list/hmdb_matches_0.4/"
...
for (i in (1:length(sps_select_l))) { 
  ...
  if(max(res_l[[i]]) > threshold) {hmdb_best_match <- hmdb_with_mz[idx[2]]}

  #' export the scoring matrix & the annotations from hmdb
  #'' get mark of hmdb
  if(max(res_l[[i]]) > threshold) {
    sps_final_l[[i]] <- sps_select[idx[1]]
  sps_final_l[[i]]$candidate_source <- "hmdb"
  }
  
  #'' export those that have best matched.
  if(max(res_l[[i]]) > threshold) {
    sps_final_l[[i]]$hmdb_candidate_score <- max(res_l[[i]])
    sps_final_l[[i]]$hmdb_candidate_hmdb_id <- hmdb_best_match$compound_id
    sps_final_l[[i]]$hmdb_candidate_predicted <- hmdb_best_match$predicted
    sps_final_l[[i]]$hmdb_candidate_spectrum_id <- hmdb_best_match$spectrum_id
