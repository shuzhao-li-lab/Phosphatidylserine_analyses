...
dataset: HILICpos reanalyzed from 04022021
date: 05152021
description: this Rmd is to recreate a less stringent significant list.
...
```{r}
getwd()
output_dir = "../../../../data/output/MS2_Spectra_massbank_match_05152021/"
dir.create(output_dir)
output_dir = "../../../../data/output/MS2_Spectra_massbank_match_05152021/HILICpos/"
dir.create(output_dir)
output_dir = "../../../../data/output/MS2_Spectra_massbank_match_05152021/HILICpos/Significant_list/"
dir.create(output_dir)

# load statistical test full report
ttest_df <- read.table("../../../../data/output/v0412_fix/HILICpos/ttest_equal/ttest_res_full_report_R5posvsR5neg.txt", 
                       header = TRUE, sep = '\t')
ttest_df[1:5,1:5]
colnames(ttest_df)
nrow(ttest_df)
...
nrow(ttest_df[ttest_df$padj < pval_thre & ttest_df$Median > Med_thre,])
```
- I decide to go with padj < 0.05 & Median of integrated intensity > 100,000 which include 781 to trim down.
...
```{r}
if(FALSE) {
  filt_df <- read.csv("../../../../data/output/HILICpos_0331analysis/X4_incl_list_merge_wt_hmdb/merged_hmdb_mcg_fit_inclusion_list.csv", header = TRUE)
}
...
```{r}
colnames(filt_df)[13:15] #G1
colnames(filt_df)[16:18] #G2
...
```{r}
filt_df$mean_G1 <- rowMeans(filt_df[,13:15], na.rm = TRUE)
filt_df$mean_G2 <- rowMeans(filt_df[,16:18], na.rm = TRUE)
...
p_or_padj <- "padj" #padj rawPval; change the $pval as well
...
### (Supplementary) Sort the table
```{r}
colnames(filt2_df)
```

```{r}
filt2_df$abs_log2FC_R5posvsR5neg <- abs(filt2_df$log2FC_R5posvsR5neg)
s_filt2_df <- filt2_df %>% 
                arrange(desc(Median),desc(abs_log2FC_R5posvsR5neg), padj)

write.csv(s_filt2_df,paste0(output_dir,"sorted_filt_med_",as.character(Med_thre),"_", p_or_padj,"_",as.character(pval_thre),"_FC_",as.character(FC_thres),"_",file_key,".csv"), row.names = FALSE)
