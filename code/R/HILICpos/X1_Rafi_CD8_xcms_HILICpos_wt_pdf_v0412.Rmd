...
output_dir1 = "../../../data/output/v0412_fix/HILICpos/"
dir.create(output_dir1)
output_dir2 = "../../../data/output/v0412_fix/HILICpos/xcms/"
...
path <- "../../../data/input/batch1/mzML/HILICpos_exp/"
list.files(path)[1:5]

metadata <- read.csv("../../../data/output/cleaned_sequence_files_collection/HILIC_seq.csv", stringsAsFactors = FALSE, header = TRUE)
colnames(metadata)
```
...
if(FALSE) { # If internal standard table have retention time; otherwise do manual input
  rtr <- c(internal_df$rt[row_number]-rt_diff,
         internal_df$rt[row_number]+rt_diff) # retention time
  if (rtr[1]<0) {rtr[1] = 0}
  if(rtr[2]>300) {rtr[2] = 300}
} else {rtr = c(0,160)} # Manual input
...
mz_diff <- round(ppm2mz_diff(ppm,internal_df$deprotonated[row_number]),4)
...
```{r}
mzr <- c(89.04098,89.04185) # 99.98 + c(-0.01, 0.01)
rtr <- c(40,80)


chr_1 <- chromatogram(filterFile(xdata, 8), mz = mzr, rt = rtr)
chr_2 <- chromatogram(filterFile(xdata_pp, 8), mz = mzr, rt = rtr)
par(mfrow = c(1, 2))
plot(chr_1)
plot(chr_2)
...
if(TRUE) {
  ...
  res <- refineChromPeaks(chr_1, MergeNeighboringPeaksParam(minProp = 0.75, expandRt = 5))
  chromPeaks(res)
  par(mfrow = c(1, 2))
  plot(chr_1)
  plot(res)
...
pdp <- PeakDensityParam(sampleGroups = xdata$sample_group,
                        minFraction = 0.7, bw = 2, binSize = 0.02) #Still need to be tested.
```
...
## Perform the correspondence
pdp <- PeakDensityParam(sampleGroups = xdata$sample_group,
                        minFraction = 0.7, bw = 2, binSize = 0.02)   # Here I use 0.7 to get at least 2 samples have the features.
...
saveRDS(xdata, file = paste(output_dir,"HILICpos_xcms_minFrac07_bw2_ppm1_sum_binSize0.02.rds",sep = ""))
# Restore the object
