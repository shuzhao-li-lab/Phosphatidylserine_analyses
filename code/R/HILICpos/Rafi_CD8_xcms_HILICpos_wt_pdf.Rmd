---
title: "Rafi_exhausted_CD8_xcms_HILICpos_simplified"
author: "Minghao Gong"
date: "3/18/2021"
...
output_dir = "../../../data/output/HILICpos_031821run/"
```


## Get the full path to the mzML files of HILICpos
```{r, results=FALSE}

folder_path = "/Users/gongm/Documents/projects/Rafi_Ahmed_exhaustedCD8Tcell/data/input/batch1/mzML/HILICpos_exp"
...
```{r}
metadata <- read.csv("../../../data/output/cleaned_sequence_files_collection/HILIC_seq.csv", stringsAsFactors = FALSE)
colnames(metadata)
```

```{r}
pd = metadata[,c("sample_name","sample_group","technical_group","method")]
...
mzr_l <- internal_df$protonated #change according to positive/negative modes

## extract the chromatogram
pdf(paste(output_dir,"chromatogram_internal_standards.pdf", sep = ""))
par(mfrow = c(3, 2))  # 3 rows and 2 columns
for (mz in mzr_l) {
  chr_raw <- chromatogram(raw_data, mz = c(mz-0.01, mz+0.01), rt = rtr)
...
#   fig_l[[numerator]] <- plot(chr_raw, col = group_colors[chr_raw$sample_group], plot = "logical")
#   numerator = numerator + 1
# }

# multi.page <- ggarrange(plotlist = fig_l,
#                         nrow = 2, ncol = 2)
# ggexport(multi.page, filename = paste(output_dir,"chromatogram_internal_standards.pdf", sep = ""))
...
mzr <- c(mzr_l[1]-0.01, mzr_l[1]+0.01)
...
write.csv(chromPeaks(xdata),"../../../data/output/RPneg_031821run/peak_picking/original_setting_peak_tab.csv")
```
...
write.csv(chromPeaks(xdata_pp),"../../../data/output/RPneg_031821run/peak_picking/refined_setting_peak_tab.csv")
```
...
pdf(paste(output_dir,"consist_pk_detect_patterns_across_samples.pdf", sep = ""))
plotChromPeakImage(xdata)
dev.off()
...
chr_serine <- chromatogram(xdata, mz = c(106.04, 106.06))
...
# Evaluation
```{r}
list_FT <- c(1:10,100:110,1000:1010)
...
saveRDS(xdata, file = paste(output_dir,"HILICpos_xcms.rds",sep = ""))
# Restore the object
